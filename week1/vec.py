...
def equal(u,v):
    ...
    for x in u.D:
        if x in u.f.keys(): 
            if x in v.f.keys():
                current=(current&(u.f[x]==v.f[x]))
            else:
                current=(current&(u.f[x]==0))
        else:
            if x in v.f.keys():
                current=(current&(v.f[x]==0))
    return current

def add(u,v):
    "Returns the sum of the two vectors"
    assert u.D == v.D
    a=v.copy()  #important in case when we add two vectors of the same name -> add(v,v) - i'm not sure - is that true?
    for x in u.D:
        if x in u.f.keys(): 
            if x in a.f.keys():
                a.f[x]=v.f[x]+u.f[x]
...
def dot(u,v):
    ...
    current=0
    for x in u.D:
        if (x in u.f.keys()) and (x in v.f.keys()):
            current=current+u.f[x]*v.f[x]
    return current
